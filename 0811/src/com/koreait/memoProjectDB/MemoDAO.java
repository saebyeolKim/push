...
//				삭제할 글의 비밀번호와 삭제하기 위해서 입력한 비밀번호를 비교해서 같으면 글을 삭제하고 다르면 false를 리턴시칸다.
				if (vo.getPassword().equals(password)) {
					
//					삭제할 글의 비밀번호와 삭제하기 위해 입력한 비밀번호가 같기 때문에 글을 삭제한다.
...
//	MemoProjectDBMain 클래스에서 수정할 글번호를 넘겨받고 글이 정상적으로 수정되면 true, 실패하면 false를 리턴하는 메소드
...
//		수정할 글의 비밀번호와 수정하기 위해서 입력한 비밀번호를 비교해서 같으면 글을 수정하고 다르면 false를 리턴시칸다.
		try {
			conn = DBUtil.getMySqlConnection();
//			수정할 글을 화면에 보여주기 위해서 삭제할 글을 테이블에서 얻어온다.
			String sql = "select * from memo where idx = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, idx);
			rs = pstmt.executeQuery();
...
//					수정할 글의 비밀번호와 수정하기 위해 입력한 비밀번호가 같기 때문에 글을 수정한다.
