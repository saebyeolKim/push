...
//	TreeSet에 저장할 클래스는 Compareable 인터페이스를 구현받고 compareTo() 메소드를 Override 한 후 객체의 정렬 기준을 지정해야 한다.
//	TreeSet에 저장할 클래스에 구현한 Compareable 인터페이스의 제네릭에는 현재 클래스의 이름을 넣어주면 된다.
...
//	비교할 데이터가 문자열일 경우 자신이 크면 1, 같으면 0, 작으면 -1을 리턴시킨다.
...
//		name을 기준으로 오름차순 정렬한다. 단 name이 같은 경우 age의 내림차순으로 정렬한다.
