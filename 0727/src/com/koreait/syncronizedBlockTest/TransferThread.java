...
//		동기화란 작업의 순서를 정한다는 의미로 공유 영역을 사용하는 스레드가 실행중일 때 같은 공유 영역을
//		사용하는 다른 스레드가 실행되지 못하도록 하는 것을 말한다. => 스레드 동기화
//		동기화 블록에는 실행 중 절대로 스레드 간에 작업 전환이 이루어지면 안되는 내용을 코딩한다.
		
		synchronized (shareArea) {
			for (int i = 0; i < 20; i++) {
				shareArea.lee.deposit(1000000);
				System.out.println("이몽룡 계좌에 100만원 입금");
				if (shareArea.hong.withDraw(1000000) != 0) {
					System.out.println("홍길동 계좌에서 100만원 출금");
				}
				try {
					sleep(100);
