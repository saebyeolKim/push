...
public class Server {
	...
	public static void main(String[] args) {
		...
		try {
//			서버를 만든다.
//			192.168.1.4의 ip 주소를 가지는 서버의 10004번 포트를 열어 서버 소켓을 생성한다.
			server = new ServerSocket(10004);
			System.out.println("서버 시작: " + server);
//			클라이언트가 접속하기를 무한정 기다린다.
//			accept() 메소드는 지정된 포트로 클라이언트가 접속할 때까지 무한 대기한다. => 클라이언트에서 넘어오는 통신 소켓을 받는다.
...
//			서버에서 클라이언트로 메시지를 전송한다. => 반드시 끝에 개행 문자(\n)를 붙여준다.
			printWriter.write("어서와 클라이언트야~ 오래기다렸어~\n");
//			flush() 메소드로 풀력 버퍼가 가득차지 않았더라도 데이터를 전송하도록 한다.
			printWriter.flush();
