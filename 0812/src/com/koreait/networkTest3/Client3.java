...
//					윈도우에서 "X"을 클릭해도 윈도우가 닫히지 않게한다.
	...
	public static void main(String[] args) {
		...
		try {
//			서버에 접속한다.
			client.socket = new Socket("192.168.1.4", 10004);
			client.message = "192.168.1.4 서버의 10004번 포트포 접속 시도 \n";
			client.message += client.socket + "접속 성공\n";
			client.textArea.setText(client.message);
			
//			서버에 접속했으므로 메시지를 입력할 수 있도록 텍스트필드로 포커스를 이동한다.
			client.textField.requestFocus();
			
//			서버와 메시지를 주고받기 위해서 테이터 전송에 사용할 객체를 선언한다.
...
//	서버에서 언제 메시지를 보내올지 모르기 때문에 통신이 종료될 때 까지 반복하며 서버에서 전송되는 메시지는 스레드로 받는다.
	@Override
	public void run() {
...
//		서버와 통신이 종료되면 메시지를 입력할 수 없도록 텍스트 필드와 전송버튼을 비활성화 시킨다.
...
//		텍스트 필드에 메시지가 입력된 상태일 경우 메시지를 클라이언트 채팅창에 표시하고 서버로 전송한다.
		if (msg.length() > 0) {
//			텍스트 필드에 입력한 메시지를 클라이언토 채팅창에 표시한다.
			message = "client>> " + msg + "\n" + message;
			textArea.setText(message);
